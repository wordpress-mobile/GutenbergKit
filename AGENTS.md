...
-   **Dependency install scripts require review.** The `allowScripts` field in `package.json` lists the packages permitted to run install scripts, pinned to the exact version that was reviewed. When `npm ci` warns that a package is "not yet covered by allowScripts" — typically after a dependency bump — read what the script does before running `npm approve-scripts <package-name>`, or `npm deny-scripts <package-name>` if the project does not need it. Do not edit `allowScripts` by hand.
