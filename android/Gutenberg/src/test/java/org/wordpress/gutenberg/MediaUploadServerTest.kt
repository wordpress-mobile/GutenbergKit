...
class MediaUploadServerTest {
    ...
    @Before
    fun setUp() {
        server = MediaUploadServer(uploadDelegate = null, defaultUploader = null)
    }
    ...
    fun `calls delegate processFile and uploadFile`() {
        ...
        server.stop()
        server = MediaUploadServer(uploadDelegate = delegate, defaultUploader = null)
    ...
    fun `falls back to default uploader when delegate returns nil for uploadFile`() {
        ...
        server.stop()
        server = MediaUploadServer(uploadDelegate = delegate, defaultUploader = mockUploader)
    ...
    fun `DefaultMediaUploader sends correct request to WP REST API`() {
        ...
        mockWpServer.enqueue(
            MockResponse()
                ...
                .setBody("""{"id":1,"source_url":"u","alt_text":"","caption":{"rendered":""},"title":{"rendered":"t"},"mime_type":"image/jpeg","media_type":"image"}""")
        )
        mockWpServer.start()
